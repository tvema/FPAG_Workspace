...
interface HeaderProps {
  ...
  filesData: Record<
    string,
    { is_modified?: boolean; type?: string; name?: string }
  >;
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="text-xs bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            disabled={!activeFile || !filesData[activeFile]?.is_modified}
            className={`text-xs px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-colors ${!activeFile || !filesData[activeFile]?.is_modified ? "text-slate-500 bg-white/5 cursor-not-allowed border border-white/5" : "bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 cursor-pointer"}`}
          >
          ...
          <button
            ...
            disabled={!Object.values(filesData).some((f) => f.is_modified)}
            className={`text-xs px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-colors ${!Object.values(filesData).some((f) => f.is_modified) ? "text-slate-500 bg-white/5 cursor-not-allowed border border-white/5" : "bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 cursor-pointer"}`}
          >
            <Save className="w-3.5 h-3.5" />
            Save All
          </button>
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content
              align="end"
              sideOffset={8}
              className="bg-[#1e1e1e] border border-white/10 rounded-md shadow-xl py-1 min-w-[140px] z-50"
            >
              <DropdownMenu.Item
                onClick={onOpenEditorSettings}
                className="px-3 py-1.5 text-xs cursor-pointer outline-none flex items-center justify-between text-slate-300 hover:bg-white/5 hover:text-white"
              >
                <span className="flex items-center gap-2">
                  <Settings2 className="w-3.5 h-3.5" /> Editor Settings
                </span>
              </DropdownMenu.Item>
              <DropdownMenu.Separator className="h-px bg-white/10 my-1" />
              <DropdownMenu.Label className="px-3 py-2 text-[10px] sm:text-xs font-semibold text-slate-500 uppercase">
                Makefile Templates
              </DropdownMenu.Label>
              <DropdownMenu.Item
              ...
              <DropdownMenu.Item
                onClick={() => handleEditTemplate("cpp")}
                className="px-3 py-1.5 text-xs cursor-pointer outline-none flex items-center justify-between text-slate-300 hover:bg-white/5 hover:text-white"
              >
